...
public class YatpaConfig {
    ...
    private YatpaConfig(
        ...
        this.teleportDisabledDimensions = teleportDisabledDimensions != null ? teleportDisabledDimensions
                : new HashSet<>();
        this.sounds = sounds;
    ...
    public static YatpaConfig from(FileConfiguration config) {
        ...
        // RTP blacklist (preferred: settings.rtp.blacklisted_worlds)
        blockedRtp.addAll(parseDimensionRestrictions(config, "settings.rtp.blacklisted_worlds"));
        // Back-compat/alias keys (accept common variations)
        blockedRtp.addAll(parseDimensionRestrictions(config, "settings.rtp.blacklistedworlds"));
        blockedRtp.addAll(parseDimensionRestrictions(config, "rtp.blacklisted_worlds"));
        blockedRtp.addAll(parseDimensionRestrictions(config, "rtp.blacklistedworlds"));
        Set<String> blockedTeleport = parseDimensionRestrictions(config,
                "settings.dimension_restrictions.disable_teleport");
        ...
        // parse per-world overrides using keys like
        // settings.costs.xp_levels.<world>.<teleport>
        ...
        if (config.isInt("settings.rtp.realm_min_distance.overworld"))
            realmMin.put("overworld", config.getInt("settings.rtp.realm_min_distance.overworld"));
        if (config.isInt("settings.rtp.realm_min_distance.nether"))
            realmMin.put("nether", config.getInt("settings.rtp.realm_min_distance.nether"));
        if (config.isInt("settings.rtp.realm_min_distance.end"))
            realmMin.put("end", config.getInt("settings.rtp.realm_min_distance.end"));
        if (config.isInt("settings.rtp.realm_max_distance.overworld"))
            realmMax.put("overworld", config.getInt("settings.rtp.realm_max_distance.overworld"));
        if (config.isInt("settings.rtp.realm_max_distance.nether"))
            realmMax.put("nether", config.getInt("settings.rtp.realm_max_distance.nether"));
        if (config.isInt("settings.rtp.realm_max_distance.end"))
            realmMax.put("end", config.getInt("settings.rtp.realm_max_distance.end"));
        ...
        return new YatpaConfig(
                ...
                config.getDouble("settings.spawn.yaw", 0),
                config.getDouble("settings.spawn.pitch", 0),
                config.getString("settings.spawn.world", "world"));
    }
    ...
    public int rtpMinDistance() {
        return rtpMinDistance;
    }

    public int rtpMaxDistance() {
    ...
    public boolean rtpDisabledIn(org.bukkit.World world) {
        return worldMatchesRestriction(world, rtpDisabledDimensions);
    }

    public boolean teleportDisabledIn(org.bukkit.World world) {
        return worldMatchesRestriction(world, teleportDisabledDimensions);
    }
    ...
    public int landingRandomOffset() {
        return landingRandomOffset;
    }
    ...
    public boolean rtpToOverworld() {
        return rtpToOverworld;
    }
    ...
    public int xpCost(TeleportKind kind) {
        return xpCosts.getOrDefault(kind, 0);
    }

    public int itemCost(TeleportKind kind) {
    ...
    public Particle effect(String key) {
